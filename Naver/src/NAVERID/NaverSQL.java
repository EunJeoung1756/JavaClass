...
//		System.out.println("con : " + con);
		...
		try {
			...
			// ▲ 회원가입 성공 실패 여부를 확인하기 위한것! 없어도 실행됨
	...
	public void select() {
		...
		try {
			...
			while(rs.next()) { // 레코드가 더이상 존재하지 않을 때까지 반복
	...
	public boolean idCheck(String id, String pw) {
		...
		try {
			...
			if(rs.next()) { // 결과 값이 존재할 때
				checkResult = true;
			} else { 		// 결과 값이 존재하지 않을 때
				checkResult = false;
			}
		} catch (SQLException e) {
...
// SELECT : rs = pstmt.executeQuery(); 이렇게!
// DELECT, INSERT 등등 : int result = pstmt.executeUpdate();
